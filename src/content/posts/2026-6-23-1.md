...
厌倦了 Among Us 官方服务器的延迟和匹配不便？想和朋友们拥有一个完全属于自己的太空狼人杀世界？Impostor 是一个开源的 Among Us 第三方服务器模拟器，能让你在 Linux 服务器上轻松搭建私服，享受低延迟、自定义规则的乐趣。
...
```bash
# 示例：下载 v1.8.0 版本（请替换为实际最新版本）
wget https://github.com/Impostor/Impostor/releases/download/v1.8.0/Impostor-v1.8.0-linux-x64.zip
```
...
Impostor 服务器默认使用 **UDP 22023** 端口。如果服务器开启了防火墙（如 UFW），需要放行该端口：

```bash
sudo ufw allow 22023/udp
```

**重要**：如果你使用的是云服务商（如阿里云、腾讯云等），还需要在云控制台的**安全组**中，添加入站规则，允许 UDP 协议的 22023 端口。
...
4.  将下载的文件放入 Among Us 游戏客户端目录中，替换原有文件。
5.  启动游戏，进入在线模式，即可看到并加入你的私人服务器。
...
- **检查 .NET 运行时**：运行 `dotnet --info` 确认 .NET 运行时已正确安装。
- **检查文件权限**：确保 `Impostor.Server` 文件具有执行权限（`chmod +x Impostor.Server`）。
- **检查端口占用**：确保 22023 端口未被其他程序占用。

### 3. 如何更新服务器版本？

1.  从 GitHub Releases 页面下载最新版本的 Linux 服务端压缩包。
2.  备份原有的 `config.json` 配置文件。
3.  解压新版本文件到目录，覆盖旧文件。
4.  将备份的 `config.json` 复制回来，覆盖新版本的配置文件。
5.  重新启动 Impostor 服务器。

## 七、结语

至此，你已经成功在 Linux 服务器上部署了一个属于你自己的 Among Us 私服
